...
class AuthViewModel : ViewModel(){
    ...
    init {
        Utlis.getAuthInstance().currentUser?.let {
    ...
    fun sendOTP(userNumber:String, activity: Activity){
        ...
        val options = PhoneAuthOptions.newBuilder(Utlis.getAuthInstance())
    ...
    fun signInWithPhoneAuthCredential(otp: String, userNumber: String, user: Users) {
        ...
        if (verificationId != null) {
            ...
            Utlis.getAuthInstance().signInWithCredential(credential)
            ...
            // For example, show an error message or retry sending OTP
        }
    }
